...
st.markdown("""
    ...
    .heartbeat { color: #10b981; font-family: 'JetBrains Mono', monospace; font-size: 0.8rem; text-align: right; margin-top: -20px; }
    </style>
...
@st.cache_data(ttl=25) # TTL reduzido para garantir rotação de dados
def fetch_and_refine_data(ticker, timestamp):
    # O 'timestamp' força o cache a invalidar se o tempo mudar
...
# --- 4. EXECUÇÃO CORE ---
try:
    # Heartbeat: Marcador de pulso de processamento
    now = datetime.now()
    st.markdown(f"<div class='heartbeat'>NEURAL HEARTBEAT: {now.strftime('%H:%M:%S')}.{int(time.time()*1000)%1000:03d}ms | SYNC: ACTIVE</div>", unsafe_allow_html=True)
    
    # Coleta passando o minuto atual para forçar atualização de cache
    data = fetch_and_refine_data("EURUSD=X", now.minute)
...
# Sincronização Forçada via Meta-Refresh + Auto-Refresh Script
st.markdown("""
    <script>
